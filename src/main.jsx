...
function pickFolder(sets) {
  var seed = getCurrentWeekSeed();
  return sets[seed % sets.length];
}

var WEEK_FOLDER = "2026W14";

function makeImages(folder) {
  return [
    { id: 1, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image1.png", label: "Scene 1", emoji: "✨" },
    { id: 2, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image2.png", label: "Scene 2", emoji: "✨" },
    { id: 3, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image3.png", label: "Scene 3", emoji: "✨" },
    { id: 4, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image4.png", label: "Scene 4", emoji: "✨" },
    { id: 5, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image5.png", label: "Scene 5", emoji: "✨" },
    { id: 6, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image6.png", label: "Scene 6", emoji: "✨" },
    { id: 7, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image7.png", label: "Scene 7", emoji: "✨" },
    { id: 8, src: CLOUDINARY_BASE+"/PixelBound/"+folder+"/image8.png", label: "Scene 8", emoji: "✨" },
  ];
}
const STORY_IMAGES = makeImages(WEEK_FOLDER);
...
.scard{background:rgba(255,255,255,.035);border:1px solid rgba(212,175,55,.13);border-radius:14px;overflow:hidden;cursor:pointer;transition:border-color .3s,transform .2s,box-shadow .3s;position:relative;display:flex;flex-direction:row;align-items:stretch;min-height:110px}
.scard:hover{border-color:rgba(212,175,55,.38);transform:translateX(4px);box-shadow:0 6px 24px rgba(0,0,0,.4)}
.scard-thumb{width:110px;min-width:110px;height:110px;object-fit:contain;background:#0d0a1a;display:block;flex-shrink:0}
...
.scard-print{background:transparent;border:1px solid rgba(212,175,55,.35);border-radius:7px;padding:3px 8px;font-family:'Cinzel',serif;font-size:9px;letter-spacing:1px;color:rgba(212,175,55,.6);cursor:pointer;display:flex;align-items:center;gap:5px;transition:all .2s;text-transform:uppercase}
.scard-edit{background:transparent;border:1px solid rgba(212,175,55,.35);border-radius:7px;padding:3px 8px;font-family:'Cinzel',serif;font-size:9px;letter-spacing:1px;color:rgba(212,175,55,.6);cursor:pointer;display:flex;align-items:center;gap:5px;transition:all .2s;text-transform:uppercase}
.scard-edit:hover{border-color:#d4af37;color:#d4af37;background:rgba(212,175,55,.1)}
...
.pbimg-card img{width:100%;height:110px;object-fit:contain;display:block;background:#0d0a1a}
...
.pbpb{display:flex}
.pbpi{width:170px;min-width:170px;height:170px;object-fit:contain;background:#0d0a1a;border-right:1px solid rgba(212,175,55,.1)}
...
.pbath{width:60px;height:60px;object-fit:contain;background:#0d0a1a;border-radius:6px;border:1px solid rgba(212,175,55,.18);flex-shrink:0}
...
.faq{position:relative;z-index:10;max-width:720px;margin:0 auto;padding:24px 24px 80px}
.faq-title{font-family:'Cinzel',serif;font-size:22px;font-weight:600;color:#d4af37;text-align:center;margin-bottom:8px;text-shadow:0 0 18px rgba(212,175,55,.3)}
.faq-sub{text-align:center;font-style:italic;color:rgba(232,213,183,.5);font-size:13px;margin-bottom:36px}
.faq-item{background:rgba(255,255,255,.035);border:1px solid rgba(212,175,55,.15);border-radius:14px;margin-bottom:14px;overflow:hidden}
.faq-q{font-family:'Cinzel',serif;font-size:13px;color:#d4af37;padding:18px 20px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:12px}
.faq-q:hover{background:rgba(212,175,55,.05)}
.faq-a{font-size:14px;color:rgba(232,213,183,.75);line-height:1.75;padding:0 20px 18px;font-style:italic}
.faq-icon{font-size:18px;color:rgba(212,175,55,.5);flex-shrink:0}
@media(min-width:768px){
.pbgrid{grid-template-columns:repeat(4,1fr)}
.pbimg-card img{height:130px}
.shelf{gap:14px}
.scard{min-height:130px}
.scard-thumb{width:130px;min-width:130px;height:130px}
...
.pbpi{width:100%;min-width:100%;height:auto;aspect-ratio:1;object-fit:contain;background:#0d0a1a;border-right:none;border-bottom:1px solid rgba(212,175,55,.1)}
...
function Homepage({ onNewStory, onRead, sessionStories, onDelete, onPrint, onFaq, onEdit, storyImages }) {
  storyImages = storyImages || STORY_IMAGES;
      ...
      const all = await Promise.all(_ks.map(async k=>{
        try{
          ...
          if(d.imageIds && !d.selectedImages){
            d.selectedImages=d.imageIds.map(id=>(storyImages||STORY_IMAGES).find(i=>i.id===id)).filter(Boolean);
          }
          return {_key:k,...d};
        }catch{ return null; }
      }));
...
function Builder({ onSaved, onCancel, editingStory, storyImages }) {
  storyImages = storyImages || STORY_IMAGES;
  ...
  useEffect(() => {
    fetch("/PixelBound/sets.json")
      ...
      .then(function(sets){
        if (sets && sets.length) {
          var seed = getCurrentWeekSeed();
          var folder = sets[seed % sets.length];
          setStoryImages(makeImages(folder));
        }
      })
      .catch(function(){});
  }, []);
  ...
  return (
    <>
      ...
      <div className="pb">
        ...
        {view==="home"&&!showFaq&&<Homepage onNewStory={()=>setView("builder")} onRead={s=>{setActiveStory(s);setReading(true);}} sessionStories={sessionStories} onDelete={handleDelete} onPrint={s=>setPrinting(s)} onFaq={()=>setShowFaq(true)} onEdit={s=>{setEditingStory(s);setView("builder");}} storyImages={storyImages}/>}
        {view==="home"&&showFaq&&<FAQ onClose={()=>setShowFaq(false)}/>}
        {view==="builder"&&<Builder onSaved={handleSaved} onCancel={()=>{setEditingStory(null);setView("home");}} editingStory={editingStory} storyImages={storyImages}/>}
      </div>
    </>
  );
}
